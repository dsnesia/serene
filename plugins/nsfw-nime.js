...
switch (type) {
	...
	case 'xwaifu':
        ...
        //conn.sendButton(m.chat, `✅ Random *${command}*`, NSgc, xwfu.url, [[`▷▷ ${msg.next()}`, `${usedPrefix + command}`]], m)
       m.react(xmoji)    
  break
...
case 'cum':
case 'xneko':
    let as = await conn.getFile(global.API('lann', `/api/nsfw-nime/${command}`, { }, 'apikey'))
...
case 'hentai':
    ...
    let les = await conn.getFile(global.API('lann', `/api/nsfw-nime/${he}`, { }, 'apikey'))
